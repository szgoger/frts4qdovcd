...
def gen_matrix_element(function_to_eval, molecule, mycc, args):
    ...
    rho = dft.numint.eval_rho(molecule, ao_value, dm1)
...
def apply_confinement_and_field(molecule,E,l):
    ...
    h=(molecule.intor('cint1e_kin_sph') + molecule.intor('cint1e_nuc_sph') + relative_confinement* molecule.intor('cint1e_r2_sph') - 0.0000 * molecule.intor('cint1e_z_sph').trace()) #Adding a confinement l*r^2
    mf = scf.UHF(molecule)
    ...
    conf = cc.UCCSD(mf)
    conf.kernel()
    r2_conf =  gen_matrix_element(rn, mol, conf,2)
    r3_conf =  gen_matrix_element(rn, mol, conf,3)
    alpha_conf = polarizability_lm_lpmp(mol, conf, 1, 0, 1, 0) #dipole alpha
...
def apply_confinement_and_quadrupole_field(molecule,E,l):
    ...
    conf = cc.UCCSD(mf)
    conf.kernel()
    r2_conf =  gen_matrix_element(rn, molecule, coupledcluster,2) # Calculating r2 in confimenent (note: no field here)
    r4_conf =  gen_matrix_element(rn, molecule, coupledcluster,4) # Calculating r2 in confimenent (note: no field here)
    confined_energy = conf.e_tot
...
for conf in confinements:
    ...
    print(conf, alpha, str(a0*np.real(alpha_l4)/np.real(al4_0)/alpha), a0*r2_conf**2/r2_0**2/alpha, a0*r3_conf /r3_0/alpha)
